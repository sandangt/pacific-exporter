...
class ExportService:
    ...
    def __add_footer(self, canvas: Canvas, doc: BaseDocTemplate):
        ...
        p_datetime.wrap(5*cm, self.__FOOTER_PARAGRAPH_HEIGHT)
        w_signature_name, h_signature_name = p_signature_name.wrap(6*cm, self.__FOOTER_PARAGRAPH_HEIGHT)
        _, h_signature_info = p_signature_info.wrap(9*cm, self.__FOOTER_PARAGRAPH_HEIGHT)

        p_datetime.drawOn(canvas, doc.width - doc.rightMargin - 6*cm, doc.bottomMargin + 5*cm)
        p_signature_name.drawOn(canvas, doc.width - doc.rightMargin - 10.35*cm + (w_signature_name / 2), doc.bottomMargin+h_signature_info)
        p_signature_info.drawOn(canvas, doc.width - doc.rightMargin - 8*cm, doc.bottomMargin)
        canvas.restoreState()
